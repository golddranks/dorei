...
from dorei.utils.audio_split import split_audio
import sys
import os
import math
import time
...
prefix = sys.prefix+"/"
bunseki_blob = open(prefix+'dorei/corpus/bunseki_blob.txt', 'rb')
...
def open_audio(filename, start, end):
	...
	filename = os.path.join(prefix, 'dorei/audio/', filename+'.m4a')
	nyt = int(time.time()*100)
	out_file = prefix+'dorei/temp/'+str(nyt)+".m4a"
	duration = split_audio(filename, start, end, out_file)
	files = os.listdir(prefix+'dorei/temp')
	for f in files:
		try:
			f_i = int(f[:-4])
		except:
...
#		words[line[0]] = 1/int(line[2])
	...
	for i, line in enumerate(open(prefix+'dorei/corpus/words_ep.txt')):
		line = line.strip('\n').split('\t')
		word = line[0]
		examples = line[3:]
		example_sentences = []
